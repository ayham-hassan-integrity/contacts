...
@section('title', __('Create Perosn'))

@section('content')
    <x-forms.post :action="route('admin.perosn.store')">
        <x-backend.card>
            <x-slot name="header">
                @lang('Create Perosn')
            </x-slot>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create Perosn')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection